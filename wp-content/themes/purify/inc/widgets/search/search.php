...
class Thim_Search_Widget extends WP_Widget {
	...
	function thim_Search_Widget() {
		//Constructor
		$widget_ops = array(
			'classname'   => 'thim_search_widget',
			'description' => 'A search form for ranger site.'
		);
		$this->WP_Widget( 'thim_search_widget', 'Thim: Search on Page', $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div id="header-search-form-input" class="main-header-search-form-input <?php echo esc_attr( $class_custom ); ?>">
			<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<input type="text" value="" name="s" id="s" placeholder="<?php esc_attr_e( 'Search...', 'thim' ); ?>" class="form-control ob-search-input" autocomplete="off" />
