...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_home() ) {
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', 'thim' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'thim' ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', 'thim' ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', 'thim' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

		echo $before . __( 'Posts tagged &ldquo;', 'thim' ) . single_tag_title( '', false ) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata( $author );
		echo $before . __( 'Author:', 'thim' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) ) {
		echo ' (' . __( 'Page', 'thim' ) . ' ' . get_query_var( 'paged' ) . ')';
	}
