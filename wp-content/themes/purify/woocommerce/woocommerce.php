...
// Override WooCommerce function
//remove_action( 'woocommerce_before_product_slider_loop_item_title', 'woocommerce_show_product_loop_sale_flash', 10 );
...
function woocommerce_category_image() {
	if ( is_product_category() ) {
		...
		if ( $image ) {
			echo '<div class="image col-md-4 col-xs-12"><img src="' . $image . '" alt="" /></div>';
		}
	}
}
...
if ( ! function_exists( 'woocommerce_checkout_payment' ) ) {
    ...
    function woocommerce_checkout_payment() {
        ...
        wc_get_template( 'checkout/payment.php', array(
            ...
            'available_gateways' => WC()->payment_gateways()->get_available_payment_gateways(),
            'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( 'Place order', 'thim' ) )
        ) );
    }
}
...
//product list/grid
add_action( 'woocommerce_before_shop_loop', 'woocommerce_product_filter', 29 );
if ( !function_exists( 'woocommerce_product_filter' ) ) {
	function woocommerce_product_filter() {
		echo '<div class="title-filter">';
					_e('Filter:','thim');
			echo '</div>';
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
    ...
    function woocommerce_catalog_ordering() {
        ...
        $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
            'popularity' => __( 'Sort by popularity', 'thim' ),
            'rating'     => __( 'Sort by average rating', 'thim' ),
            'date'       => __( 'Sort by newness', 'thim' ),
            'price'      => __( 'Sort by price: low to high', 'thim' ),
            'price-desc' => __( 'Sort by price: high to low', 'thim' )
        ) );
...
if ( !function_exists( 'woocommerce_result_count' ) ) {
	function woocommerce_result_count() {
		...
		$pc   = !empty( $params['product_count'] ) ? $params['product_count'] : $per_page;
		$html = '<div class="limit"><b>' . __( 'Show:', 'purify' ) . '</b>';
		$html .= '<select class="limit-number" onchange="location = this.value;">
...
remove_action( 'woocommerce_single_product_summary_quick', 'woocommerce_template_single_price', 10 );
add_action( 'woocommerce_single_product_summary_quick', 'woocommerce_template_single_price', 15 );
...
remove_action( 'woocommerce_single_product_summary_quick', 'woocommerce_template_loop_add_to_cart_quick_view', 20 );
add_action( 'woocommerce_single_product_summary_quick', 'woocommerce_template_loop_add_to_cart_quick_view', 40 );
...
if ( !function_exists( 'woocommerce_template_loop_add_to_cart_quick_view' ) ) {
	function woocommerce_template_loop_add_to_cart_quick_view( $args = array() ) {
		wc_get_template( 'loop/add-to-cart-quick-view.php', $args );
	}
}
...
function thim_add_to_cart_success_ajax( $count_cat_product ) {
	...
	if ( $cart_items > 0 ) {
		$cart_items = $cart_items . __( ' items', 'thim' );
	} else {
...
function wooshare() {
	...
	if ( $theme_options_data['thim_woo_sharing_facebook'] == 1 ||
        ...
        $theme_options_data['thim_woo_sharing_instagram'] == 1 ||
        $theme_options_data['thim_woo_sharing_youtube'] == 1
	) {
        ...
        if ( $theme_options_data['thim_woo_sharing_youtube'] == 1 ) {
            $html .= '<li><a target="_blank" class="youtube" href="https://youtube.com/share?url=' . urlencode( get_permalink() ) . '&amp;title=' . esc_attr(get_the_title()) . '" title="' . __( 'Youtube', 'thim' ) . '" onclick=\'javascript:window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;\'><i class="fa fa-youtube-play"></i></a></li>';
        }

        if ( $theme_options_data['thim_woo_sharing_instagram'] == 1 ) {
            $html .= '<li><a target="_blank" class="instagram" href="https://instagram.com/share?url=' . urlencode( get_permalink() ) . '&amp;title=' . esc_attr(get_the_title()) . '" title="' . __( 'Instagram', 'thim' ) . '" onclick=\'javascript:window.open(this.href, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600");return false;\'><i class="fa fa-instagram"></i></a></li>';
        }
...
function list_grid_ajax_url() {
	echo '<script type="text/javascript">
		var ob_ajax_url ="' . get_site_url() . '/wp-admin/admin-ajax.php";
		</script>';
}

add_action( 'wp_print_scripts', 'list_grid_ajax_url' );

add_action( 'wp_ajax_list_grid', 'list_grid' );
add_action( 'wp_ajax_nopriv_list_grid', 'list_grid' );
function list_grid() {
	global $post, $product, $woocommerce;
	$prod_id = $_POST["product"];
	$post    = get_post( $prod_id );
